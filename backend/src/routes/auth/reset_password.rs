...
// the get has the code inside
// the post route for reset-password also has the code inside the url
pub async fn new_password(mut req: Request<Arc<TideState>>) -> tide::Result {
    return Response::empty().into_response();
}
