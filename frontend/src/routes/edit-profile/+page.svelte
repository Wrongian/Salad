...
<div class="bg-lime-50 px-6">
  <div class="font-medium text-2xl px-4 py-3 relative flex justify-start border-lime-100 border-b-2 border-solid">
  ...
  <div class = "flex justify-start py-4 px-4  rounded-md border-lime-100 border-b-2 border-solid">
    <!--Avatar Div-->
    <div class="px-4 pt-2 relative">
      <button  on:click={() => {isModalShown = true}} >
      <Avatar.Root class="w-[100px] h-[100px] ring-2 ring-lime-300 ">
        <Avatar.Image class="z-4 hover:brightness-50 peer-hover/image" src={imageURL} alt="">
        </Avatar.Image>
    ...
    <div class="justify-start my-auto ">
        <!--Display name form-->
        <div class="mb-2 py-4 ">
          <label for="change-name" class="block mb-2 font-medium text-xl text-gray-900">Change your Name:</label>
          <input on:focusout={() => {submitDisplayName()}} type="text" id="change-name" bind:value={displayNameData} class="block font-medium px-0 w-full text-2xl text-gray-900 border-lime-300 bg-transparent border-b-2 focus:outline-none focus:ring-0 focus:border-lime-600" placeholder="" />
        </div>
      <div on:focusout={() => {submitBio()}} class = "mb-2 py-4">
        <!--Bio Form-->
