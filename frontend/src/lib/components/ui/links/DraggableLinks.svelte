...
>
  ...
  <ul class="max-w-md">
    ...
    title={link.title} bio={link.bio} imageLink={link.imageLink}></EditableLink> -->
      ...
      <div
        id="linkdiv-{listData.linkData.id}"
        on:dragover|preventDefault={(e) => {
          dragOver(e, listData, index);
        }}
        on:dragend={async (e) => {
          await endDrag(e, listData, index);
        }}
        on:dragstart={(e) => {
          startDrag(e, listData, index);
        }}
      >
        <Link {index} {deleteCallback} {modalCallback} {listData}></Link>
      </div>
    {/each}
  </ul>
</div>
...
{#if isModalShown}
  <PictureModal
    bind:isModalShown
    imageSubmitFunction={updateLinkImage}
    modalText="Upload Link Picture"
  ></PictureModal>
{/if}
