...
<div
  on:focusout={() => {
    handleModalFocusLoss;
  }}
  class="overflow-y-auto overflow-x-hidden z-50 fixed top-0 right-0 left-0 justify-center items-center w-full md:inset-0 h-full flex"
  tabindex="-1"
>
  <div class="relative p-4 w-full max-w-md max-h-full">
    <!-- Modal content -->
    <div class="relative bg-white rounded-lg shadow">
      <span class="inline-block m-4 text-gray-500 text-medium text-xl"
        >{modalText}</span
      >
      <button
        on:click={() => {
        ...
        type="button"
        class="absolute top-4 end-3 bg-transparent hover:bg-gray-200 hover:text-gray-900 w-8 h-8 inline-flex justify-center items-center"
      >
        <svg
          ...
          <div class="max-w-xl bg-gray-50 rounded-lg shadow-xl">
            <div class="m-4">
              <div class="flex items-center justify-center w-full">
                <label
                  class="flex-col flex w-full h-full border-4 border-lime-400 border-dashed rounded hover:bg-gray-100 hover:border-lime-300"
                >
